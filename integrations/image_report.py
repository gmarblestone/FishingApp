...
def generate_image_string(forecast) -> Image.Image:
    ...
    # ── Footer ───────────────────────────────────────────────────────────────
    footer_y = CARD_H - 50
    draw.text((40, footer_y), f"Grant's Fishing Forecast v1.5.4 | NOAA / NDBC / NWS | {forecast.generated_at}", fill="#64748b", font=f_tiny)
...
def generate_detail_image(forecast, day_indices, title, output_path):
    ...
    # Footer
    draw.text((40, y + 10), f"Grant's Fishing Forecast v1.5.4 | NOAA / NDBC / NWS | {forecast.generated_at}", fill="#64748b", font=f_tiny)
